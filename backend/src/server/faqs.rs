use crate::{
    entities::{self, faqs},
    models::{
        AddFaqRequest, DeleteFaqQuery, GetFaqsQuery, SyphonError, SyphonResult, UpdateFaqRequest,
    },
    utils::db::db,
};
use actix_web::{
...
pub async fn create_faqs(req_body: web::Json<AddFaqRequest>) -> SyphonResult<HttpResponse> {
    let db = db();
...
pub async fn update_faqs(req_body: web::Json<UpdateFaqRequest>) -> SyphonResult<HttpResponse> {
    let db = db();
...
pub async fn list_faqs(query: Query<GetFaqsQuery>) -> SyphonResult<HttpResponse> {
    let db = db();
...
pub async fn delete_faqs(query: Query<DeleteFaqQuery>) -> SyphonResult<HttpResponse> {
    let db = db();
