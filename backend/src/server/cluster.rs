...
pub async fn join_cluster(
    ...
    let request = entities::requests::Entity::find_by_id(body.request_id)
        .one(db).await?
        .ok_or(SyphonError::Json(json!("request not found"), StatusCode::NOT_FOUND))?;

    if !request.is_clusterable {
        return Err(SyphonError::Json(json!("request is not clusterable"), StatusCode::BAD_REQUEST));
    }
    ...
    match existing_cluster {
        Some(cluster) => {
            if cluster.cluster_id == body.cluster_id {
                ...
                Err(SyphonError::Json(json!("request already in another cluster"), StatusCode::BAD_REQUEST))
            }
        },
        None => {
...
pub async fn edit_cluster(
    ...
    let cluster = entities::clusters::Entity::find()
        ...
        .one(db).await?
        .ok_or(SyphonError::Json(json!("cluster not found"), StatusCode::NOT_FOUND))?;
