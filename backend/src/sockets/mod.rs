...
impl SocketChannels {
    ...
    pub async fn is_allowed(&self, zid: i32) -> bool {
        let validator: fn(i32, i32) -> bool = match self {
            SocketChannels::Notifications(q) => Self::is_allowed_notifications,
            SocketChannels::QueueData(_) => Self::is_allowed_queue_data,
            SocketChannels::Request(_) => Self::is_allowed_request,
            SocketChannels::Announcements(_) => Self::is_allowed_announcements,
            SocketChannels::Chat(_) => Self::is_allowed_chat,
        };
        validator(self.inner_id(), zid).await
    }
