...
const QueueRequestsSummaryCard = ({ requests }: QueueRequestsSummaryCardProps) => {
  

  return <>
    <Card className={styles.infoCard}>
      <div className={styles.infoCardTitleContainer}>
        <Typography className={styles.summaryHeadings}  variant='h5'>Time Per Request:</Typography>
      </div>
      <div className={styles.allSummaryItemsContainer}>

        <div className={styles.tagsContainer}>
          {requests.map((request: QueueRequestSummaryData ) => {
            return <div className={styles.summaryItem} key={request.requestId}>
              <TagBox
              ...
              /> 
              {request.duration ? 
                <Typography variant='body1'>{getActualDuration(request.duration)?.minutes.toString() + ' mins ' + getActualDuration(request.duration)?.seconds.toString() + ' seconds'}</Typography>
                :
                <Typography variant='body1'>This request was not resolved by a tutor</Typography>
              }
            </div>;
          })}
        </div>
